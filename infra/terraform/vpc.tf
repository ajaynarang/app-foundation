...
resource "aws_subnet" "public" {
  ...
  tags = { Name = "${local.prefix}-subnet-public-${var.availability_zones[count.index]}" }
}
...
resource "aws_subnet" "private" {
  ...
  tags = { Name = "${local.prefix}-subnet-private-${var.availability_zones[count.index]}" }
}

# NAT Gateway (allows private subnet outbound internet for ECR pulls, Anthropic API, etc.)
# single_nat_gateway=true: 1 shared NAT (staging, saves ~$35/month, no HA)
# single_nat_gateway=false: 1 NAT per AZ (production, full HA)
locals {
...
resource "aws_security_group" "elasticache" {
  ...
  tags = { Name = "${local.prefix}-sg-elasticache" }
}
