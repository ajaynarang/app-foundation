 ...
 *
 * `toolTier` is the scope tier at execution time — derived from the
 * tool's @RequiresScope via ScopeRegistryService. We persist it on the
 * step row for audit (cheaper than recomputing later if the scope
 * vocabulary shifts). `toolScope` is the full `domain:action[:sensitive]`
 * string, also frozen at execution.
 */
