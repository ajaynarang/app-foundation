 ...
 *
 * The Crew tab consumes `AgentRosterItem`. The agent sheet consumes
 * `AgentDetail`. PATCH /desk/agents/:key accepts `UpdateAgentRequest`.
 */

// ─── Supervisor (tenant user) ───────────────────────────────────────────
// Inlined so the agent-detail file doesn't reach into the user domain.
...
export const AgentRosterItemSchema = z.object({
  ...
  name: z.string(),
  description: z.string().nullable(),
  isActive: z.boolean(), // any enabled AVAILABLE responsibility?
...
// ─── Full agent detail — consumed by the sheet header + tabs ────────────
...
// ─── Windowed activity stats (GET /desk/agents/:key/activity) ───────────
...
// ─── Eligible supervisors (GET /desk/agents/eligible-supervisors) ───────
