...
/**
 * Foundation event catalog — every event the AppShore foundation itself emits.
 *
 * Do NOT add product events here — add them to APP_EVENT_REGISTRY in
 * `apps/backend/src/platform-glue/events/event-registry.ts`, which merges this
 * catalog with yours. Event keys are namespaced `app.<aggregate>.<verb>`; the
 * in-process subscribers listen on the `app.**` wildcard.
 */
export const FOUNDATION_EVENT_REGISTRY = [
  // ─── Notifications ─────────────────────────────────────────────────
  {
...
/**
 * Derive a `DOMAIN_EVENTS`-style constant map from a registry, preserving
 * literal string types: `DOMAIN_EVENTS.USER_CREATED` is typed `'app.user.created'`.
 */
export function buildDomainEvents<T extends readonly EventDefinition[]>(registry: T) {
  return Object.fromEntries(registry.map((e) => [e.constantName, e.key])) as {
    [K in T[number] as K['constantName']]: K['key'];
  };
}

/** Constant map for the foundation catalog only — safe to use from @appshore/platform. */
export const FOUNDATION_DOMAIN_EVENTS = buildDomainEvents(FOUNDATION_EVENT_REGISTRY);
