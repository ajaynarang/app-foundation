...
describe('AppCacheService', () => {
  ...
  describe('get', () => {
    ...
    it('should record hit/miss metrics', async () => {
      mockRedis.get.mockResolvedValue(null);
      await service.get('app:test:key1');
      mockRedis.get.mockResolvedValue(JSON.stringify('data'));
      await service.get('app:test:key2');

      const metrics = service.getMetrics();
      expect(metrics['app:test'].misses).toBe(1);
      expect(metrics['app:test'].hits).toBe(1);
    });
  });

  describe('set', () => {
    it('should JSON-stringify value and store with PX TTL', async () => {
      await service.set('app:test:key', { foo: 'bar' }, 60000);
      expect(mockRedis.set).toHaveBeenCalledWith('app:test:key', JSON.stringify({ foo: 'bar' }), 'PX', 60000);
    });

    it('should store null sentinel for null values', async () => {
      await service.set('app:test:key', null, 60000);
      expect(mockRedis.set).toHaveBeenCalledWith('app:test:key', '__APP_NULL__', 'PX', 60000);
    });

    it('should store null sentinel for undefined values', async () => {
      await service.set('app:test:key', undefined, 60000);
      expect(mockRedis.set).toHaveBeenCalledWith('app:test:key', '__APP_NULL__', 'PX', 60000);
    });
  });
  ...
  describe('del', () => {
    it('should delete from Redis', async () => {
      await service.del('app:test:key');
      expect(mockRedis.del).toHaveBeenCalledWith('app:test:key');
    });
  });
  ...
  describe('getOrSet', () => {
    it('should return cached value without calling factory', async () => {
      ...
      const result = await service.getOrSet('app:test:key', factory, 60000);
    ...
    it('should call factory and cache result on miss (lock acquired)', async () => {
      ...
      const cacheSet = allSetCalls.find((c) => c[0] === 'app:test:key');
  ...
  describe('delByPrefix', () => {
    it('should scan and delete every key under the prefix', async () => {
      mockRedis.scan.mockResolvedValueOnce(['0', ['app:tower:wire:1:alert:0:50', 'app:tower:wire:1:desk:0:50']]);
      mockRedis.del.mockResolvedValue(2);

      const count = await service.delByPrefix('app:tower:wire:1:');

      expect(mockRedis.scan).toHaveBeenCalledWith('0', 'MATCH', 'app:tower:wire:1:*', 'COUNT', 200);
      expect(mockRedis.del).toHaveBeenCalledWith('app:tower:wire:1:alert:0:50', 'app:tower:wire:1:desk:0:50');
      expect(count).toBe(2);
    });

    it('should walk the SCAN cursor until it returns to 0', async () => {
      mockRedis.scan
        .mockResolvedValueOnce(['7', ['app:tower:active-loads:1:4']])
        .mockResolvedValueOnce(['0', ['app:tower:active-loads:1:8']]);
      mockRedis.del.mockResolvedValue(1);

      const count = await service.delByPrefix('app:tower:active-loads:1:');
  ...
  describe('flushNamespace', () => {
    it('should scan and delete keys matching pattern', async () => {
      mockRedis.scan.mockResolvedValueOnce(['0', ['app:test:key1', 'app:test:key2']]);
      mockRedis.del.mockResolvedValue(2);

      const count = await service.flushNamespace('app:test' as any);
      expect(count).toBe(2);
    });
  });
  ...
  describe('increment', () => {
    it('should INCRBY then EXPIRE on first touch only', async () => {
      ...
      await service.increment('app:test:counter', 1, 60);
      expect(mockRedis.incrby).toHaveBeenCalledWith('app:test:counter', 1);
      expect(mockRedis.expire).toHaveBeenCalledWith('app:test:counter', 60);
    });
