'use client';

import { useState, useCallback, useEffect } from 'react';
import { Plus, X } from 'lucide-react';
import { Button } from '@sally/ui/components/ui/button';
import { Input } from '@sally/ui/components/ui/input';
import { Label } from '@sally/ui/components/ui/label';
import { Checkbox } from '@sally/ui/components/ui/checkbox';
import { Badge } from '@sally/ui/components/ui/badge';
import { cn } from '@sally/ui';
import { FormSheet } from '@/shared/components/ui/form-sheet';
import { useUpdateCustomFieldDefinition } from '../hooks/use-custom-field-definitions';
import type { CustomFieldDefinition } from '../types';

const TYPE_LABELS: Record<string, string> = {
  TEXT: 'Text',
  NUMBER: 'Number',
  DATE: 'Date',
  SELECT: 'Select',
};

interface EditFieldSheetProps {
  definition: CustomFieldDefinition;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

interface FormState {
  name: string;
  options: string[];
  isRequired: boolean;
  driverEditable: boolean;
  showOnInvoice: boolean;
  showOnBol: boolean;
}

export function EditFieldSheet({ definition, open, onOpenChange }: EditFieldSheetProps) {
  const [form, setForm] = useState<FormState>({
    name: definition.name,
    options: definition.options && definition.options.length > 0 ? definition.options : [''],
    isRequired: definition.isRequired,
    driverEditable: definition.driverEditable,
    showOnInvoice: definition.showOnInvoice,
    showOnBol: definition.showOnBol,
  });
  const [nameError, setNameError] = useState<string | null>(null);
  const [optionErrors, setOptionErrors] = useState<Record<number, string>>({});

  const updateMutation = useUpdateCustomFieldDefinition();

  // Sync form when definition changes or sheet opens
  useEffect(() => {
    if (open) {
      setForm({
        name: definition.name,
        options: definition.options && definition.options.length > 0 ? definition.options : [''],
        isRequired: definition.isRequired,
        driverEditable: definition.driverEditable,
        showOnInvoice: definition.showOnInvoice,
        showOnBol: definition.showOnBol,
      });
      setNameError(null);
      setOptionErrors({});
    }
  }, [open, definition]);

  const handleFieldChange = useCallback(<K extends keyof FormState>(key: K, value: FormState[K]) => {
    setForm((prev) => ({ ...prev, [key]: value }));
    if (key === 'name') setNameError(null);
  }, []);

  const addOption = () => {
    setForm((prev) => ({ ...prev, options: [...prev.options, ''] }));
  };

  const updateOption = (index: number, value: string) => {
    setForm((prev) => {
      const options = [...prev.options];
      options[index] = value;
      return { ...prev, options };
    });
    setOptionErrors((prev) => ({ ...prev, [index]: '' }));
  };

  const removeOption = (index: number) => {
    setForm((prev) => ({
      ...prev,
      options: prev.options.filter((_, i) => i !== index),
    }));
  };

  const validate = useCallback((): boolean => {
    let valid = true;
    if (!form.name.trim()) {
      setNameError('Field name is required');
      valid = false;
    }
    if (definition.fieldType === 'SELECT') {
      const errs: Record<number, string> = {};
      form.options.forEach((o, i) => {
        if (!o.trim()) errs[i] = 'Option cannot be empty';
      });
      if (Object.keys(errs).length > 0) {
        setOptionErrors(errs);
        valid = false;
      }
      if (form.options.filter((o) => o.trim()).length === 0) {
        setOptionErrors({ 0: 'At least one option is required' });
        valid = false;
      }
    }
    return valid;
  }, [form.name, definition.fieldType, form.options]);

  const handleSubmit = useCallback(() => {
    if (!validate()) return;

    const options = definition.fieldType === 'SELECT' ? form.options.filter((o) => o.trim()) : undefined;

    updateMutation.mutate(
      {
        id: definition.id,
        data: {
          name: form.name.trim(),
          ...(options !== undefined ? { options } : {}),
          isRequired: form.isRequired,
          driverEditable: form.driverEditable,
          showOnInvoice: form.showOnInvoice,
          showOnBol: form.showOnBol,
        },
      },
      { onSuccess: () => onOpenChange(false) },
    );
  }, [form, definition, updateMutation, onOpenChange, validate]);

  return (
    <FormSheet
      open={open}
      onOpenChange={onOpenChange}
      title="Edit Custom Field"
      mode="edit"
      onSubmit={handleSubmit}
      submitLabel="Save Changes"
      isSubmitting={updateMutation.isPending}
    >
      <div className="space-y-6">
        {/* Field Name */}
        <div className="space-y-2">
          <Label htmlFor="edit-field-name">
            Field Name <span className="text-destructive">*</span>
          </Label>
          <Input
            id="edit-field-name"
            value={form.name}
            onChange={(e) => handleFieldChange('name', e.target.value)}
            className={cn(nameError && 'border-destructive')}
          />
          {nameError && <p className="text-xs text-destructive">{nameError}</p>}
        </div>

        {/* Field key (read-only) */}
        <div className="space-y-2">
          <Label>Field Key</Label>
          <div className="flex items-center gap-2">
            <code className="flex-1 text-sm font-mono bg-muted text-muted-foreground rounded-md px-3 py-2 border border-border">
              {definition.fieldKey}
            </code>
            <p className="text-xs text-muted-foreground">Immutable</p>
          </div>
        </div>

        {/* Field type (immutable) */}
        <div className="space-y-2">
          <Label>Field Type</Label>
          <div className="flex items-center gap-2">
            <Badge variant="muted">
              {TYPE_LABELS[definition.fieldType as string] ?? (definition.fieldType as string)}
            </Badge>
            <p className="text-xs text-muted-foreground">Cannot be changed after creation</p>
          </div>
        </div>

        {/* SELECT options — editable */}
        {definition.fieldType === 'SELECT' && (
          <div className="space-y-2">
            <Label>Options</Label>
            <div className="space-y-2">
              {form.options.map((opt, i) => (
                <div key={i} className="space-y-1">
                  <div className="flex gap-2">
                    <Input
                      placeholder={`Option ${i + 1}`}
                      value={opt}
                      onChange={(e) => updateOption(i, e.target.value)}
                      className={cn(optionErrors[i] && 'border-destructive', 'flex-1')}
                    />
                    {form.options.length > 1 && (
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-10 w-10 shrink-0 text-muted-foreground hover:text-destructive"
                        onClick={() => removeOption(i)}
                        aria-label="Remove option"
                      >
                        <X className="h-4 w-4" />
                      </Button>
                    )}
                  </div>
                  {optionErrors[i] && <p className="text-xs text-destructive">{optionErrors[i]}</p>}
                </div>
              ))}
              <Button variant="outline" size="sm" onClick={addOption} className="w-full">
                <Plus className="h-3.5 w-3.5 mr-1.5" />
                Add Option
              </Button>
            </div>
          </div>
        )}

        {/* Behavior checkboxes */}
        <div className="space-y-3">
          <Label>Behavior</Label>
          {[
            { key: 'isRequired' as const, label: 'Required', description: 'Must be filled before saving' },
            { key: 'driverEditable' as const, label: 'Driver visible', description: 'Shown in driver app' },
            { key: 'showOnInvoice' as const, label: 'Show on invoice', description: 'Printed on invoices' },
            { key: 'showOnBol' as const, label: 'Show on BOL', description: 'Printed on bill of lading' },
          ].map(({ key, label, description }) => (
            <div key={key} className="flex items-start gap-3">
              <Checkbox
                id={`edit-${key}`}
                checked={form[key]}
                onCheckedChange={(checked) => handleFieldChange(key, !!checked)}
                className="mt-0.5"
              />
              <div>
                <Label htmlFor={`edit-${key}`} className="cursor-pointer font-normal">
                  {label}
                </Label>
                <p className="text-xs text-muted-foreground">{description}</p>
              </div>
            </div>
          ))}
        </div>
      </div>
    </FormSheet>
  );
}
