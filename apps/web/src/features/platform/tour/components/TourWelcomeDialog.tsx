    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md overflow-hidden border-0 bg-gradient-to-b from-card to-background p-0">
        ...
        <div className="relative flex flex-col items-center gap-5 px-8 py-10">
          {/* Sally Orb — real deal, not an emoji */}
          <motion.div
            initial={{ scale: 0, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ type: 'spring', stiffness: 200, damping: 15, delay: 0.1 }}
          >
            <SallyOrb state="listening" size="lg" />
          </motion.div>
          ...
          >
            <DialogTitle className="text-2xl font-bold text-foreground tracking-tight">Meet SALLY</DialogTitle>
