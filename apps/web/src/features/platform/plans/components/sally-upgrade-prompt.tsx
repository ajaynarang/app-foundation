...
interface SallyUpgradePromptProps {
  feature: string;
}

export function SallyUpgradePrompt({ feature }: SallyUpgradePromptProps) {
...
// ---------------------------------------------------------------------------
// Add-on upsell card — "Request from SALLY"
// ---------------------------------------------------------------------------
          ...
          <div className="flex-1 bg-card border border-border rounded-xl p-5">
            ...
            {canManageBilling ? (
              <div className="flex gap-3">
                <a
                  href={isPaymentMode ? upgradeUrl : mailto('sally', `Upgrade to ${config.requiredPlan}`)}
                  className="inline-flex items-center justify-center rounded-md text-sm font-medium h-10 px-4 py-2 bg-primary text-primary-foreground hover:bg-primary/90 transition-colors"
                >
                ...
                {!isPaymentMode && (
                  <a
                    href={mailto('sally', `Learn more about ${config.displayName}`)}
