...
/**
 * Dynamic follow-up suggestions surfaced after a Sally response. Renders
 * nothing when the store has no follow-ups — generic capability prompts
 * now live in the command palette (SallyCommandPalette), not here.
 */
export function SallyActionBar({ onSelect, hideTopBorder = false }: SallyActionBarProps) {
  const followUps = useSallyStore((s) => s.suggestedFollowUps);
  ...
  return (
    <div className={`shrink-0 ${hideTopBorder ? '' : 'border-t border-border/50'}`}>
      ...
      <div className="relative group/bar">
        ...
        <div
          ref={scrollRef}
          className="overflow-x-auto px-3 pb-2 sally-hide-scrollbar"
          style={{
            scrollbarWidth: 'none',
