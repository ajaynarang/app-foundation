...
export function AssistantStrip({ hideOrb }: { hideOrb?: boolean } = {}) {
  const {
    isExpanded,
    orbState,
    userMode,
  ...
  const handleDragMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      document.documentElement.style.setProperty('--assistant-side-width', `${newWidth}px`);
    },
    [isDragging],
  );
  ...
  useEffect(() => {
    if (isExpanded && chatLayout === 'side') {
      document.documentElement.style.setProperty('--assistant-side-width', `${sideWidth}px`);
    } else {
      document.documentElement.style.removeProperty('--assistant-side-width');
    }
  ...
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      ...
      if (e.key === 'Escape' && isExpanded) {
        ...
        // picker (AssistantInput handles that); the panel must stay open.
      ...
      <AnimatePresence>
        {isExpanded ? (
          <motion.div
            ...
            className={`${panelClasses} ${chatLayout === 'float' ? 'assistant-float-glow' : 'bg-background'} flex flex-col overflow-hidden ${isDragging || isFloatDragging ? '' : 'transition-[width,inset,transform,border-radius,box-shadow] duration-300 ease-out'}`}
          >
            {/* Float glow inner wrapper — only in float mode */}
            <div className={chatLayout === 'float' ? 'assistant-float-inner' : 'flex flex-col flex-1 min-h-0'}>
          ...
          /* Floating orb — bottom-right corner. Quiet by default; nerve net
             ...
             inside the orb (see AssistantOrb) so we don't render a sibling chip. */
          !hideOrb && (
            <motion.div
              key="collapsed"
            ...
            >
              <AssistantOrb state={orbState} size="lg" onClick={expandStrip} hasUnread={hasUnreadAsync} showHint />
            </motion.div>
          )
        )}
      </AnimatePresence>
