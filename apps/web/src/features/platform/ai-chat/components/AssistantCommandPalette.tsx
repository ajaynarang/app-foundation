...
export function AssistantCommandPalette({ open, onOpenChange, onPickPrompt }: AssistantCommandPaletteProps) {
  const setDraftInput = useAssistantStore((s) => s.setDraftInput);
  const expandStrip = useAssistantStore((s) => s.expandStrip);
  ...
  // burn a Assistant turn, and quick-action templates may need tweaking.
  ...
  // Restore focus to the Assistant chat textarea when the palette closes.
