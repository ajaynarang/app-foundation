...
/** Grace period before a scheduled job's status indicator shows warning */
...
function StatusIndicator({ row }: { row: FlatRow }) {
  ...
  if (row.lastRunStatus === 'COMPLETED') {
    // Check staleness: warn only if a scheduled nextRun was missed
    if (row.schedule && row.schedule !== 'Manual' && row.nextRun) {
      const nextRunTime = new Date(row.nextRun).getTime();
      const isOverdue = Date.now() > nextRunTime + SCHEDULE_OVERDUE_GRACE_MS;
      if (isOverdue) {
            ...
            {filteredRows.map((row) => (
              ...
              >
                ...
                <TableCell className="text-right hidden sm:table-cell text-muted-foreground">
                  ...
                  {row.todayFailed > 0 && <span className="text-critical ml-1">({row.todayFailed} failed)</span>}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
