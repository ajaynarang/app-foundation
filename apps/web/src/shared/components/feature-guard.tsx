...
export function FeatureGuard({ entitlementKey, children }: FeatureGuardProps) {
      ...
      <Card className="max-w-md w-full">
        <CardContent className="pt-8 pb-8 text-center space-y-4">
          ...
          {canManageBilling ? (
            ...
            <p className="text-xs text-muted-foreground mt-2">Ask your account admin to upgrade your plan.</p>
          )}
        </CardContent>
      </Card>
    </div>
      ...
      <div className="max-w-lg w-full text-center space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          {isTrialExpired && canManageBilling && (
            <a
              href={isPaymentMode ? upgradeUrl : '/pricing'}
            ...
            >
              {isPaymentMode ? 'Upgrade Now' : 'View Plans & Pricing'}
            </a>
          )}
          <a
