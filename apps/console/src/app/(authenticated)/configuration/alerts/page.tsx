...
import { getAlertConfig, updateAlertConfig } from '@/features/settings/api';
import type { AlertConfiguration } from '@/features/settings/api';
...
const ESCALATION_TARGETS = [
  ...
  { value: 'all_dispatchers', label: 'All Dispatchers' },
  { value: 'admin', label: 'Admin' },
];
const ESCALATION_CHANNEL_OPTIONS = ['email', 'sms', 'push', 'in_app'] as const;
const ESCALATION_CHANNEL_LABELS: Record<string, string> = {
  email: 'Email',
  sms: 'SMS',
  push: 'Push',
  in_app: 'In-App',
...
export default function AlertsSettingsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showError('Failed to save alert configuration', err instanceof Error ? err.message : String(err));
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Configure when and how your organization&apos;s alerts are triggered. Individual users can customize their
            own notification preferences separately.
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure how unacknowledged alerts are escalated. When an alert is not acknowledged within the SLA window,
            it escalates to the configured audience via the selected channels.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {PRIORITY_LEVELS.map((priority) => {
            const policy = alertConfig.escalationPolicy?.[priority];
            if (!policy && priority !== 'critical' && priority !== 'high') return null;
            const currentPolicy = policy || {
              acknowledgeSlaMinutes: priority === 'critical' ? 5 : 15,
              escalateTo: 'supervisors',
              channels: ['email'],
            };

            const updatePolicy = (field: string, value: unknown) => {
              setAlertConfig({
                ...alertConfig,
                escalationPolicy: {
                  ...alertConfig.escalationPolicy,
                  [priority]: { ...currentPolicy, [field]: value },
                },
              });
            };

            return (
              <div key={priority} className="space-y-3 pb-4 border-b border-border last:border-0 last:pb-0">
                <p className="text-sm font-medium capitalize text-foreground">{priority}</p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-1.5">
                    <Label className="text-xs">Acknowledge SLA (minutes)</Label>
                    <Input
                      type="number"
                      min="1"
                      max="1440"
                      className="h-9"
                      disabled={!canEdit}
                      value={currentPolicy.acknowledgeSlaMinutes}
                      onChange={(e) => updatePolicy('acknowledgeSlaMinutes', parseInt(e.target.value) || 5)}
                    />
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-xs">Escalate To</Label>
                    <Select
                      value={currentPolicy.escalateTo}
                      onValueChange={(v) => updatePolicy('escalateTo', v)}
                      disabled={!canEdit}
                    >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex flex-wrap gap-3 pt-1">
                      {ESCALATION_CHANNEL_OPTIONS.map((ch) => (
                        <Label key={ch} className="flex items-center gap-1.5 cursor-pointer">
                          <Checkbox
                            checked={currentPolicy.channels?.includes(ch) ?? false}
                            onCheckedChange={(checked) => {
                              const channels = currentPolicy.channels ?? [];
                              const updated = checked ? [...channels, ch] : channels.filter((c: string) => c !== ch);
                              updatePolicy('channels', updated);
                            }}
                            disabled={!canEdit}
                          />
                          <span className="text-xs text-muted-foreground">{ESCALATION_CHANNEL_LABELS[ch]}</span>
                        </Label>
                      ))}
                    </div>
                  </div>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Group similar alerts from different drivers in the same area (e.g., multiple weather delays).
              </p>
            </div>
            <Switch
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Automatically link related alerts (e.g., HOS approaching &rarr; HOS violation) so you see the chain.
              </p>
            </div>
            <Switch
