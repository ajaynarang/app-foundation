...
Every Platform API error returns a consistent JSON structure with an HTTP status code, a human-readable message, and an error classification. This page documents the format, all status codes, and common error scenarios.
...
**Common causes:**
- User role lacks the required permission (e.g., a MEMBER trying to manage API keys)
...
**Resolution:** Confirm that the authenticated user has the appropriate role.
...
**Common causes:**
- Invalid state transition (e.g., revoking an API key that is already revoked)
- A business constraint that cannot be satisfied with the given input
...
{
  "statusCode": 422,
  "message": "Cannot revoke API key: key is already revoked",
  "error": "Unprocessable Entity"
}
```

**Resolution:** Review the business rule described in the message and adjust the input data to satisfy the constraint.
...
| Production (`sk_live_`) | 10,000 requests/hour |
...
**Resolution:** Implement exponential backoff with jitter. Monitor the `X-RateLimit-Remaining` header and throttle requests as you approach the limit. If you consistently need higher limits, contact [Support](/docs/resources/support).
...
**Resolution:** Retry the request after a brief delay. If the error persists, report it to [Support](/docs/resources/support) with the request details, timestamp, and any `requestId` from the response headers.
