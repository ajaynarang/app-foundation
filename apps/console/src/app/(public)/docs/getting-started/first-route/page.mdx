...
This tutorial walks you through planning a complete, HOS-compliant route from scratch. You will create a driver, register a vehicle, set up loads, plan the route, and activate it -- all through the Platform API.

## What You Will Build

A multi-stop route for driver Mike Johnson hauling freight from Chicago, IL through Indianapolis, IN to Columbus, OH. Platform will optimize the stop sequence, insert rest and fuel stops as needed, and validate HOS compliance at every segment.
...
- **Challenge:** The driver has been on duty for 4 hours already; Platform will determine if rest stops are required

## Prerequisites

- A Platform account ([sign up here](https://app.appshore.in))
- An API key or JWT token ([get your API key](/docs/getting-started/api-keys))
...
```bash
curl -X POST $APP_BASE/auth/login \
...
The `fuelTankCapacity` and `mpg` fields are critical -- Platform uses them to calculate when fuel stops are needed along the route.
...
Now bring everything together. Pass the driver, vehicle, and load IDs to the route planning endpoint. Platform will optimize the stop sequence, simulate HOS compliance segment by segment, and automatically insert rest and fuel stops where needed.
...
Platform returns a detailed route plan with every segment the driver will follow:
...
### Why Platform Inserted a Rest Stop

After segment 3, the driver has been 8.9 hours since their last break. FMCSA regulation 395.3 requires a 30-minute break before the 8th hour of driving. Platform detected that the driver would exceed this threshold during the next drive segment and proactively inserted a 30-minute break.
...
Once active, Platform begins continuous monitoring -- checking for HOS violations, route deviations, weather changes, and mechanical alerts every 60 seconds.

## Step 8: Check for Alerts

After activating the route, check for any alerts that Platform has generated:

```bash
curl "$APP_BASE/alerts?driverId=drv_a1b2c3d4&status=ACTIVE" \
  -H "X-API-Key: $APP_API_KEY"
```
...
If the driver deviates from the plan or an HOS threshold approaches, Platform generates alerts automatically. You can subscribe to real-time alerts using [Server-Sent Events](/docs/api-guides/alerts-monitoring/real-time-events).
...
```bash
curl $APP_BASE/routes/rte_f8e7d6c5 \
  -H "X-API-Key: $APP_API_KEY"
```
...
const BASE_URL = "https://api-staging.appshore.in/api/v1";
...
async function planFirstRoute() {
  ...
  const activateRes = await fetch(
    ...
    { method: "POST", headers: { "X-API-Key": APP_API_KEY } }
  );
...
- [Drivers API](/docs/api-guides/fleet-management/drivers) -- Manage driver profiles and HOS data
- [Vehicles API](/docs/api-guides/fleet-management/vehicles) -- Fleet vehicle management
- [Alerts and Monitoring](/docs/api-guides/alerts-monitoring) -- Monitor active routes and handle alerts
- [Real-time Events](/docs/api-guides/alerts-monitoring/real-time-events) -- Subscribe to live route updates
- [Route Planning Guide](/docs/api-guides/route-planning/creating-routes) -- Advanced route planning options
