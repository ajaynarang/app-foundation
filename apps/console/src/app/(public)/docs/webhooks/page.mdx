...
All events use the `app.*` prefix:

| Event | When it fires |
|-------|--------------|
| `app.load.created` | A new load is added to the system |
| `app.load.updated` | A load's status, stops, or assignment changes |
| `app.route.planned` | A route plan is created for a driver |
| `app.alert.fired` | An alert is generated (HOS violation, delay, deviation) |
...
### `app.load.created` / `app.load.updated`
