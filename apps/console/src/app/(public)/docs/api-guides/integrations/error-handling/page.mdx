...
This guide covers how to handle errors from the Platform API, implement effective retry strategies, and stay within rate limits. Building resilient integrations means planning for failures from the start.

## HTTP Status Codes

Platform uses standard HTTP status codes:
...
| `503` | Service Unavailable | Platform is temporarily down (retry) |
...
These indicate a problem on Platform's side. Retry with backoff.
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (response.status >= 400 && response.status < 500 && response.status !== 429) {
        ...
        throw new AppApiError(response.status, error.message, error.details);
      }
      ...
      if (response.status >= 500) {
        if (attempt === maxRetries) {
          const error = await response.json();
          throw new AppApiError(response.status, error.message);
        }
...
class AppClient:
    def __init__(self, api_key, base_url="https://api-staging.appshore.in/api/v1"):
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if 400 <= response.status_code < 500 and response.status_code != 429:
                    raise AppApiError(response.status_code, response.json())
...
Platform API endpoints have the following timeout characteristics:
...
class AppClient {
  constructor(apiKey) {
    this.apiKey = apiKey;
    this.baseUrl = "https://api-staging.appshore.in/api/v1";
  }
  ...
  // Usage examples:
  // const client = new AppClient(process.env.APP_API_KEY);
...
- [API Keys](/docs/getting-started/api-keys) -- Set up authentication
- [Webhooks](/docs/api-guides/integrations/webhooks) -- Event-driven notifications (reduce polling)
