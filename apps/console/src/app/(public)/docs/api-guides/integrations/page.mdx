...
```bash
curl https://sally-api-staging.appshore.in/api/v1/integrations \
...
const response = await fetch(
  "https://sally-api-staging.appshore.in/api/v1/integrations",
  {
...
```bash
curl https://sally-api-staging.appshore.in/api/v1/integrations/vendors \
  -H "X-API-Key: $SALLY_API_KEY"
```
...
```javascript
const response = await fetch(
  "https://sally-api-staging.appshore.in/api/v1/integrations/vendors",
  {
    headers: { "X-API-Key": process.env.SALLY_API_KEY },
  }
);
...
```bash
curl -X POST https://sally-api-staging.appshore.in/api/v1/integrations \
  -H "X-API-Key: $SALLY_API_KEY" \
...
```bash
curl -X POST https://sally-api-staging.appshore.in/api/v1/integrations/int_s1a2m3s4/test \
  -H "X-API-Key: $SALLY_API_KEY"
```
...
```bash
curl -X POST https://sally-api-staging.appshore.in/api/v1/integrations/int_s1a2m3s4/sync \
  -H "X-API-Key: $SALLY_API_KEY"
```
...
- [Webhooks](/docs/api-guides/integrations/webhooks) -- Receive event notifications from SALLY
