...
Platform's integration framework connects your fleet operations with external services -- ELD providers for HOS data, TMS platforms for load management, fuel price feeds, and weather services. Integrations run as managed adapters that Platform syncs on your behalf.

## Supported Vendors

Platform provides pre-built adapters for the following vendor categories:
...
```bash
curl https://api-staging.appshore.in/api/v1/integrations \
...
```bash
curl https://api-staging.appshore.in/api/v1/integrations/vendors \
  -H "X-API-Key: $APP_API_KEY"
```
...
```javascript
const response = await fetch(
  "https://api-staging.appshore.in/api/v1/integrations/vendors",
  {
    headers: { "X-API-Key": process.env.APP_API_KEY },
  }
);
...
```bash
curl -X POST https://api-staging.appshore.in/api/v1/integrations/int_s1a2m3s4/sync \
  -H "X-API-Key: $APP_API_KEY"
```
...
When an integration syncs, data flows into Platform and is used by the planning and monitoring engines:
...
- [ELD / Samsara Integration](/docs/api-guides/integrations/eld-samsara) -- Connect Samsara for real-time HOS and telematics
- [TMS Integration](/docs/api-guides/integrations/tms) -- Connect McLeod or Project44 for load management
- [Webhooks](/docs/api-guides/integrations/webhooks) -- Receive event notifications from Platform
- [Error Handling](/docs/api-guides/integrations/error-handling) -- Handle errors, retries, and rate limits
