...
Connecting Samsara to Platform enables automatic Hours of Service (HOS) data sync, eliminating the need to manually provide driver HOS state when planning routes. Platform pulls HOS data, vehicle locations, and diagnostic codes from Samsara every 15 minutes (configurable).

## What You Get

Once the Samsara integration is active, Platform automatically:
...
- **Matches drivers** -- Links Samsara driver profiles to Platform driver records by license number or name
...
- Admin or Owner role in Platform
...
4. Name the token (e.g., "Platform Integration")
...
## Step 2: Create the Integration in Platform
...
Verify that Platform can reach Samsara with your credentials:

```bash
curl -X POST https://api-staging.appshore.in/api/v1/integrations/int_s1a2m3s4/test \
  -H "X-API-Key: $APP_API_KEY"
```
...
```bash
curl https://api-staging.appshore.in/api/v1/drivers/drv_a1b2c3d4/hos \
  -H "X-API-Key: $APP_API_KEY"
```
...
## How Platform Uses Samsara Data

### Route Planning

When a Samsara integration is active, you no longer need to provide the `driverState` field in route planning requests. Platform automatically fetches the latest HOS data from the last sync:
...
With Samsara connected, Platform's monitoring service uses real-time HOS data to:
...
Platform matches Samsara drivers to Platform driver records using the following logic:

1. **License number match** (primary) -- If the CDL number in Samsara matches a Platform driver
2. **Name match** (secondary) -- If first and last name match exactly
3. **Manual override** -- You can manually link a Samsara driver ID to a Platform driver
...
```bash
curl -X PATCH https://api-staging.appshore.in/api/v1/integrations/int_s1a2m3s4 \
  -H "X-API-Key: $APP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
- Verify that driver license numbers in Platform match those in Samsara
...
```bash
curl https://api-staging.appshore.in/api/v1/integrations/int_s1a2m3s4 \
  -H "X-API-Key: $APP_API_KEY"
```
...
- [Drivers API](/docs/api-guides/fleet-management/drivers) -- View synced HOS data
- [TMS Integration](/docs/api-guides/integrations/tms) -- Connect McLeod or Project44
