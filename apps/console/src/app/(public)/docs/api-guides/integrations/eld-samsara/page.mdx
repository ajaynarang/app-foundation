...
```bash
curl -X PATCH https://sally-api-staging.appshore.in/api/v1/integrations/int_s1a2m3s4 \
  -H "X-API-Key: $SALLY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl https://sally-api-staging.appshore.in/api/v1/integrations/int_s1a2m3s4 \
  -H "X-API-Key: $SALLY_API_KEY"
```
