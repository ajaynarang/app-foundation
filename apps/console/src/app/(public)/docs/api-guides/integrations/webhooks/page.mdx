...
|---|---|---|
| `sally.alert.fired` | An alert has been generated | Full alert object |
| `sally.route.planned` | A new route plan was created | Plan ID, summary |
| `sally.load.created` | A new load is added | Full load object |
| `sally.load.updated` | A load's status or assignment changes | Load ID, updated fields |

## Registering a Webhook

```bash
curl -X POST https://sally-api-staging.appshore.in/api/v1/webhooks \
...
const response = await fetch(
  "https://sally-api-staging.appshore.in/api/v1/webhooks",
  {
    method: "POST",
    headers: {
...
```bash
curl https://sally-api-staging.appshore.in/api/v1/webhooks \
  -H "X-API-Key: $SALLY_API_KEY"
```
...
```bash
curl -X PUT https://sally-api-staging.appshore.in/api/v1/webhooks/wh_a1b2c3d4 \
  -H "X-API-Key: $SALLY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X DELETE https://sally-api-staging.appshore.in/api/v1/webhooks/wh_a1b2c3d4 \
  -H "X-API-Key: $SALLY_API_KEY"
```
