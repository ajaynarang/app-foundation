...
The Vehicles API lets you register trucks in your fleet and maintain their specifications. Vehicle data -- especially fuel tank capacity and fuel efficiency -- directly impacts how Platform plans routes and determines when fuel stops are needed.

## Why Vehicle Data Matters

When Platform plans a route, it uses the vehicle's `fuelTankCapacity` and `mpg` to simulate fuel consumption along each segment. If the vehicle will run low before the next stop, Platform automatically inserts a fuel stop at the most cost-effective station along the route.

A vehicle with a 150-gallon tank at 6.2 MPG has a range of approximately 930 miles on a full tank. Platform factors in the current fuel level and plans fuel stops before the tank drops below a safe reserve threshold.

## Create a Vehicle

```bash
curl -X POST https://api-staging.appshore.in/api/v1/vehicles \
...
const response = await fetch(
  "https://api-staging.appshore.in/api/v1/vehicles",
  {
    method: "POST",
    headers: {
...
```bash
curl "https://api-staging.appshore.in/api/v1/vehicles?status=ACTIVE&page=1&pageSize=25" \
  -H "X-API-Key: $APP_API_KEY"
```
...
const response = await fetch(
  `https://api-staging.appshore.in/api/v1/vehicles?${params}`,
  {
...
```bash
curl https://api-staging.appshore.in/api/v1/vehicles/veh_e5f6g7h8 \
  -H "X-API-Key: $APP_API_KEY"
```
...
```javascript
const response = await fetch(
  "https://api-staging.appshore.in/api/v1/vehicles/veh_e5f6g7h8",
  {
    headers: { "X-API-Key": process.env.APP_API_KEY },
  }
);
...
```bash
curl -X PUT https://api-staging.appshore.in/api/v1/vehicles/veh_e5f6g7h8 \
  -H "X-API-Key: $APP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "mpg": 6.0,
...
```bash
curl -X DELETE https://api-staging.appshore.in/api/v1/vehicles/veh_e5f6g7h8 \
  -H "X-API-Key: $APP_API_KEY"
```
...
When planning a route, you reference the vehicle by its ID. Platform reads the vehicle's fuel specifications to determine:
...
In this example, the vehicle has 75 gallons (50% full), giving it approximately 465 miles of range. If the route exceeds that distance, Platform inserts a fuel stop.
...
- [Drivers API](/docs/api-guides/fleet-management/drivers) -- Manage driver profiles
- [Loads API](/docs/api-guides/fleet-management/loads) -- Create shipment loads for route planning
- [Your First Route](/docs/getting-started/first-route) -- Combine drivers, vehicles, and loads into a route
