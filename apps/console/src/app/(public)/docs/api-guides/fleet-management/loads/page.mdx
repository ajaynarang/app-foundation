...
```bash
curl -X POST https://sally-api-staging.appshore.in/api/v1/loads \
...
const response = await fetch(
  "https://sally-api-staging.appshore.in/api/v1/loads",
  {
    method: "POST",
    headers: {
...
```bash
curl "https://sally-api-staging.appshore.in/api/v1/loads?status=PENDING&page=1&pageSize=25" \
  -H "X-API-Key: $SALLY_API_KEY"
```
...
const response = await fetch(
  `https://sally-api-staging.appshore.in/api/v1/loads?${params}`,
  {
...
```bash
curl https://sally-api-staging.appshore.in/api/v1/loads/load_q7r8s9t0 \
  -H "X-API-Key: $SALLY_API_KEY"
```
...
```javascript
const response = await fetch(
  "https://sally-api-staging.appshore.in/api/v1/loads/load_q7r8s9t0",
  {
    headers: { "X-API-Key": process.env.SALLY_API_KEY },
  }
);
...
```bash
curl -X PUT https://sally-api-staging.appshore.in/api/v1/loads/load_q7r8s9t0 \
  -H "X-API-Key: $SALLY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X DELETE https://sally-api-staging.appshore.in/api/v1/loads/load_q7r8s9t0 \
  -H "X-API-Key: $SALLY_API_KEY"
```
