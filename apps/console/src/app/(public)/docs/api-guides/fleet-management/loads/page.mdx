...
Loads represent individual shipments in Platform. Each load has an origin, a destination, scheduled pickup/delivery times, and cargo details. When you plan a route, you assign one or more loads to it, and Platform sequences the stops optimally while maintaining HOS compliance.
...
```bash
curl -X POST https://api-staging.appshore.in/api/v1/loads \
...
const response = await fetch(
  "https://api-staging.appshore.in/api/v1/loads",
  {
    method: "POST",
    headers: {
...
Providing `originLat`/`originLon` and `destinationLat`/`destinationLon` coordinates is strongly recommended. If omitted, Platform geocodes the addresses, which may add latency to route planning requests.

## List Loads

```bash
curl "https://api-staging.appshore.in/api/v1/loads?status=PENDING&page=1&pageSize=25" \
  -H "X-API-Key: $APP_API_KEY"
```
...
const response = await fetch(
  `https://api-staging.appshore.in/api/v1/loads?${params}`,
  {
...
```bash
curl https://api-staging.appshore.in/api/v1/loads/load_q7r8s9t0 \
  -H "X-API-Key: $APP_API_KEY"
```
...
```javascript
const response = await fetch(
  "https://api-staging.appshore.in/api/v1/loads/load_q7r8s9t0",
  {
    headers: { "X-API-Key": process.env.APP_API_KEY },
  }
);
...
```bash
curl -X PUT https://api-staging.appshore.in/api/v1/loads/load_q7r8s9t0 \
  -H "X-API-Key: $APP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
If a load is already `ASSIGNED` to a route and you change delivery times or addresses, Platform's monitoring service will detect the change and trigger a route re-plan automatically.

## Delete (Cancel) a Load

```bash
curl -X DELETE https://api-staging.appshore.in/api/v1/loads/load_q7r8s9t0 \
  -H "X-API-Key: $APP_API_KEY"
```
...
- [Drivers API](/docs/api-guides/fleet-management/drivers) -- Manage your driver fleet
- [Vehicles API](/docs/api-guides/fleet-management/vehicles) -- Register fleet vehicles
- [Your First Route](/docs/getting-started/first-route) -- Combine loads, drivers, and vehicles into routes
- [Route Planning](/docs/api-guides/route-planning/creating-routes) -- Advanced route planning with load optimization
