...
The Drivers API lets you create and manage driver profiles in your fleet. Each driver record includes personal information, license details, and real-time Hours of Service (HOS) data that Platform uses for route planning and compliance monitoring.
...
```bash
curl -X POST https://api-staging.appshore.in/api/v1/drivers \
...
const response = await fetch(
  "https://api-staging.appshore.in/api/v1/drivers",
  {
    method: "POST",
    headers: {
...
```bash
curl "https://api-staging.appshore.in/api/v1/drivers?status=ACTIVE&page=1&pageSize=25" \
  -H "X-API-Key: $APP_API_KEY"
```
...
const response = await fetch(
  `https://api-staging.appshore.in/api/v1/drivers?${params}`,
  {
...
```bash
curl https://api-staging.appshore.in/api/v1/drivers/drv_a1b2c3d4 \
  -H "X-API-Key: $APP_API_KEY"
```
...
```javascript
const response = await fetch(
  "https://api-staging.appshore.in/api/v1/drivers/drv_a1b2c3d4",
  {
    headers: { "X-API-Key": process.env.APP_API_KEY },
  }
);
...
```bash
curl -X PUT https://api-staging.appshore.in/api/v1/drivers/drv_a1b2c3d4 \
  -H "X-API-Key: $APP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST https://api-staging.appshore.in/api/v1/drivers/drv_a1b2c3d4/deactivate \
  -H "X-API-Key: $APP_API_KEY"
```
...
```bash
curl -X DELETE https://api-staging.appshore.in/api/v1/drivers/drv_a1b2c3d4 \
  -H "X-API-Key: $APP_API_KEY"
```
...
```bash
curl -X POST https://api-staging.appshore.in/api/v1/drivers/drv_a1b2c3d4/reactivate \
  -H "X-API-Key: $APP_API_KEY"
```
...
Retrieve the current Hours of Service data for a driver. This is the data Platform uses to validate compliance when planning routes.
...
```javascript
const response = await fetch(
  "https://api-staging.appshore.in/api/v1/drivers/drv_a1b2c3d4/hos",
  {
    headers: { "X-API-Key": process.env.APP_API_KEY },
  }
);
...
Drivers in Platform can be linked to user accounts with the `DRIVER` role. When a user with the `DRIVER` role logs in, they see the driver-specific view with their assigned routes and HOS information.
...
- [Vehicles API](/docs/api-guides/fleet-management/vehicles) -- Manage your vehicle fleet
- [Loads API](/docs/api-guides/fleet-management/loads) -- Create and manage shipment loads
- [Your First Route](/docs/getting-started/first-route) -- Use driver data to plan routes
- [ELD / Samsara Integration](/docs/api-guides/integrations/eld-samsara) -- Automatic HOS data sync
