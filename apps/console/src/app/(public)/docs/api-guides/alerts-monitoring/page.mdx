...
Platform continuously monitors every active route in your fleet, checking for potential issues every 60 seconds. When something needs attention -- an approaching HOS limit, a route deviation, severe weather ahead, or a dock delay -- Platform generates an alert and delivers it to dispatchers in real time.

## How Monitoring Works

Once you [activate a route](/docs/getting-started/first-route#step-7-activate-the-route), Platform's monitoring service begins tracking it across 20 alert types in 6 categories. The monitoring loop runs continuously:
...
Platform organizes alerts into six categories:
...
Alerts can be auto-resolved by Platform when the underlying condition clears. For example, if a driver was deviating from the route but returns to the planned path, the route deviation alert resolves automatically.
...
```bash
curl "https://api-staging.appshore.in/api/v1/alerts?status=ACTIVE" \
...
const response = await fetch(
  "https://api-staging.appshore.in/api/v1/alerts?status=ACTIVE",
  {
...
See [Real-time Events](/docs/api-guides/alerts-monitoring/real-time-events) for implementation details.

## Alert Analytics

Platform provides analytics endpoints to help you understand alert patterns and improve operations:

```bash
# Get alert statistics
curl "https://api-staging.appshore.in/api/v1/alerts/stats" \
  -H "X-API-Key: $APP_API_KEY"
```
...
- [Alert Types](/docs/api-guides/alerts-monitoring/alert-types) -- Detailed reference for all alert categories and types
- [Real-time Events](/docs/api-guides/alerts-monitoring/real-time-events) -- SSE and WebSocket setup for live alerts
- [Alert Management](/docs/api-guides/alerts-monitoring/alert-management) -- Acknowledge, resolve, snooze, and analyze alerts
