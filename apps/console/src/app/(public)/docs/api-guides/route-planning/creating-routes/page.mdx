...
- **Rest stops** — When HOS simulation detects the driver cannot complete a segment. See [Rest Stops](/docs/api-guides/route-planning/rest-stops).
- **Fuel stops** — When the vehicle's fuel level drops below 25% capacity. See [Fuel Stops](/docs/api-guides/route-planning/fuel-stops).
- **Breaks** — When the driver has driven 8 hours since last break (30-minute FMCSA requirement).
- **Stop sequence** — Stops are reordered by the TSP optimizer for minimum drive time. See [Stop Optimization](/docs/api-guides/route-planning/stop-optimization).
