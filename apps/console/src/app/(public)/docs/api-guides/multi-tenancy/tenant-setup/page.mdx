...
Platform is a multi-tenant platform. Every organization (carrier, broker, fleet operator) operates within its own isolated tenant. All data -- drivers, vehicles, loads, routes, and alerts -- is automatically scoped to your tenant. Users never see data from other tenants.
...
```bash
curl -X POST https://api-staging.appshore.in/api/v1/tenants/register \
...
const response = await fetch(
  "https://api-staging.appshore.in/api/v1/tenants/register",
  {
    method: "POST",
...
After registration, the tenant enters `PENDING_APPROVAL` status. A Platform administrator reviews the registration and either approves or rejects it. Approval typically takes less than 24 hours during business days.

You can check your tenant status by logging in:

```bash
curl -X POST https://api-staging.appshore.in/api/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{
    "email": "robert.williams@acmefreight.com",
    "password": "a_secure_password_here"
...
After your tenant is approved, complete these steps to start using Platform:
...
```bash
curl -X POST https://api-staging.appshore.in/api/v1/api-keys \
...
See [API Keys](/docs/getting-started/api-keys) for details.
...
```bash
curl -X POST https://api-staging.appshore.in/api/v1/user-invitations \
...
- [Drivers API](/docs/api-guides/fleet-management/drivers) -- Create driver profiles
- [Vehicles API](/docs/api-guides/fleet-management/vehicles) -- Register trucks

### 4. Configure Integrations

Connect your ELD, TMS, and other systems:

- [ELD / Samsara](/docs/api-guides/integrations/eld-samsara) -- Automatic HOS data
- [TMS Integration](/docs/api-guides/integrations/tms) -- Load sync from McLeod or Project44
- [Integrations Overview](/docs/api-guides/integrations) -- All vendor options

### 5. Plan Your First Route

With drivers, vehicles, and loads in place, plan your first route:

- [Your First Route](/docs/getting-started/first-route) -- Step-by-step tutorial
...
```bash
curl -X PUT https://api-staging.appshore.in/api/v1/tenant/settings/alerts \
  -H "X-API-Key: $APP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X PUT https://api-staging.appshore.in/api/v1/tenant/settings/monitoring \
  -H "X-API-Key: $APP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
{
  "statusCode": 403,
  "message": "Tenant tnt_acme001 is suspended. Contact app-support@appshore.in for assistance.",
  "error": "Forbidden"
}
```
...
- [User Roles and Permissions](/docs/api-guides/multi-tenancy/user-roles) -- Understand the role hierarchy
- [API Keys](/docs/getting-started/api-keys) -- Generate keys for your integrations
- [Your First Route](/docs/getting-started/first-route) -- Start planning routes
