...
```bash
curl -X POST https://sally-api-staging.appshore.in/api/v1/tenants/register \
...
const response = await fetch(
  "https://sally-api-staging.appshore.in/api/v1/tenants/register",
  {
    method: "POST",
...
```bash
curl -X POST https://sally-api-staging.appshore.in/api/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{
    "email": "robert.williams@acmefreight.com",
    "password": "a_secure_password_here"
...
```bash
curl -X POST https://sally-api-staging.appshore.in/api/v1/api-keys \
...
```bash
curl -X POST https://sally-api-staging.appshore.in/api/v1/user-invitations \
...
```bash
curl -X PUT https://sally-api-staging.appshore.in/api/v1/tenant/settings/alerts \
  -H "X-API-Key: $SALLY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X PUT https://sally-api-staging.appshore.in/api/v1/tenant/settings/monitoring \
  -H "X-API-Key: $SALLY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
{
  "statusCode": 403,
  "message": "Tenant tnt_acme001 is suspended. Contact sally-support@appshore.in for assistance.",
  "error": "Forbidden"
}
```
