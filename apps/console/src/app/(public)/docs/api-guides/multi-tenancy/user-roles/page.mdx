...
Platform uses role-based access control (RBAC) to manage what each user can do within a tenant. Every user is assigned exactly one role, and that role determines their access to API endpoints and dashboard features.
...
|---|---|---|
| `SUPER_ADMIN` | Platform-level administrator (Platform staff only) | Platform support team |
...
| `CUSTOMER` | Customer portal access for shipment visibility | Shipper, broker |
...
const response = await fetch(
  "https://api-staging.appshore.in/api/v1/user-invitations",
  {
    method: "POST",
    headers: {
...
```bash
curl -X POST https://api-staging.appshore.in/api/v1/user-invitations/accept \
  -H "Content-Type: application/json" \
  -d '{
...
```json
{
  ...
  "message": "Account created successfully. Welcome to Platform."
}
```
...
```bash
curl "https://api-staging.appshore.in/api/v1/user-invitations?status=PENDING" \
  -H "X-API-Key: $APP_API_KEY"
```
...
curl https://api-staging.appshore.in/api/v1/routes/driver/drv_a1b2c3d4/active \
...
```bash
curl -X PUT https://api-staging.appshore.in/api/v1/users/usr_t5g6a7r8/role \
  -H "X-API-Key: $APP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "role": "ADMIN"
...
```javascript
const response = await fetch(
  "https://api-staging.appshore.in/api/v1/users/usr_t5g6a7r8/role",
  {
    method: "PUT",
    headers: {
      "X-API-Key": process.env.APP_API_KEY,
...
- `SUPER_ADMIN` can only be assigned by other `SUPER_ADMIN` users (Platform staff)
...
{
  "statusCode": 400,
  "message": "Cannot assign SUPER_ADMIN role. Only Platform administrators can assign this role.",
  "error": "Bad Request"
}
```
...
- [Tenant Setup](/docs/api-guides/multi-tenancy/tenant-setup) -- Register and configure your organization
- [API Keys](/docs/getting-started/api-keys) -- Generate keys for your integrations
- [Drivers API](/docs/api-guides/fleet-management/drivers) -- Start building your fleet
