...
const response = await fetch(
  "https://sally-api-staging.appshore.in/api/v1/user-invitations",
  {
    method: "POST",
    headers: {
...
```bash
curl -X POST https://sally-api-staging.appshore.in/api/v1/user-invitations/accept \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl "https://sally-api-staging.appshore.in/api/v1/user-invitations?status=PENDING" \
  -H "X-API-Key: $SALLY_API_KEY"
```
...
curl https://sally-api-staging.appshore.in/api/v1/routes/driver/drv_a1b2c3d4/active \
...
curl "https://sally-api-staging.appshore.in/api/v1/alerts?status=ACTIVE" \
...
```bash
curl -X PUT https://sally-api-staging.appshore.in/api/v1/users/usr_t5g6a7r8/role \
  -H "X-API-Key: $SALLY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "role": "ADMIN"
...
```javascript
const response = await fetch(
  "https://sally-api-staging.appshore.in/api/v1/users/usr_t5g6a7r8/role",
  {
    method: "PUT",
    headers: {
      "X-API-Key": process.env.SALLY_API_KEY,
