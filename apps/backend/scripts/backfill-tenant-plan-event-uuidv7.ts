/**
 * Phase 2 Task 13b — TenantPlanEvent id (CUID) → UUIDv7 backfill.
 *
 * Run AFTER migration `20260514000000_tenant_plan_event_id_v7_add` has applied
 ...
 *
 * Run BEFORE migration `20260514000001_tenant_plan_event_id_v7_promote`
 * (which assumes id_v7 is fully populated before swapping the PK).
 *
 * Usage:
 *   doppler run -- pnpm tsx apps/backend/scripts/backfill-tenant-plan-event-uuidv7.ts
 *
  ...
  try {
    const rows = await prisma.$queryRaw<Array<{ id: string; created_at: Date }>>`
      SELECT id, created_at
        FROM tenant_plan_events
       WHERE id_v7 IS NULL
       ORDER BY created_at ASC
    `;
    console.log(`Backfilling ${rows.length} tenant_plan_events rows...`);

    for (const r of rows) {
      const v7 = uuidv7FromTimestamp(r.created_at);
      await prisma.$executeRaw`
        UPDATE tenant_plan_events
           SET id_v7 = ${v7}::uuid
         WHERE id = ${r.id}
      `;
    }
    ...
    const nulls = await prisma.$queryRaw<Array<{ count: bigint }>>`
      SELECT COUNT(*)::bigint AS count FROM tenant_plan_events WHERE id_v7 IS NULL
    `;
    if (nulls[0].count > 0n) {
      throw new Error(`Backfill incomplete: ${nulls[0].count} rows still NULL`);
    }
    console.log('Backfill complete. id_v7 populated for every tenant_plan_events row.');
  } finally {
