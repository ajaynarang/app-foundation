...
export class ConversationSessionService {
  ...
  async issue(conversationId: number, input: { expiresAt?: string }) {
    ...
    await this.events.emit(SALLY_EVENTS.CONVERSATION_SESSION_ISSUED, conversation.tenantId ?? 0, {
      conversationId: conversation.id,
  ...
  async revoke(sessionId: number) {
    ...
    await this.events.emit(SALLY_EVENTS.CONVERSATION_SESSION_REVOKED, session.tenantId ?? 0, {
