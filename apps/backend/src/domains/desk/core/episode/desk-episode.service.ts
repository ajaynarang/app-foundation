...
export class DeskEpisodeService {
  ...
  async getForTenant(tenantId: number, episodeId: string): Promise<DeskEpisodeDetail> {
    ...
    return {
      ...
      // instead of a machine key like ar_followup-bot. Null when the agent
...
function toListItem(row: EpisodeWithRels): DeskEpisodeListItem {
  return {
    ...
    responsibilityKey: row.responsibility.key as DeskEpisodeListItem['responsibilityKey'],
...
function toStepRecord(s: {
  ...
  return {
    ...
    gateDecision: (s.gateDecision as GateDecisionRecord) ?? null,
...
function toHandledListItem(row: HandledRow): HandledListItem {
  ...
  return {
    ...
    entityId: row.entityId,
    entitySubtitle: null,
    agentKey: row.ownerAgent.key as AgentKey,
    agentName: row.ownerAgent.name,
    responsibilityKey: row.responsibility.key as ResponsibilityKey,
    responsibilityTitle: row.responsibility.title,
