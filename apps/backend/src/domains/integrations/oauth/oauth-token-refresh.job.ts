...
export class OAuthTokenRefreshJob implements OnModuleInit {
  ...
  private async clearExistingRefreshJobs() {
    try {
      const repeatableJobs = await this.vendorDataQueue.getRepeatableJobs();
      for (const job of repeatableJobs) {
        if (job.name === VENDOR_DATA_JOB_NAMES.OAUTH_REFRESH) {
          await this.vendorDataQueue.removeRepeatableByKey(job.key);
        }
      }
    } catch (err) {
