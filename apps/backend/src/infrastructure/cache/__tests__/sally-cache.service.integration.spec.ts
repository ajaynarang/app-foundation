...
describe('SallyCacheService (integration, real Redis)', () => {
  ...
  it('writes a literal sally:* key with the exact name (no Keyv prefix, no transformation)', async () => {
    if (!available) return;
  ...
  it('flushNamespace removes only keys under that namespace', async () => {
    ...
    await service.set('sally:itest-other:keep', 'should-remain', 60_000);
    ...
    try {
      ...
      expect(await probe.get('sally:itest-other:keep')).toBeTruthy();
    } finally {
      // Clean up the bystander we wrote above so we don't leak between tests.
      await probe.del('sally:itest-other:keep');
    }
  });
