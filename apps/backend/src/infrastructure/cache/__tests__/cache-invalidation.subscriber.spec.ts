...
describe('CacheInvalidationSubscriber', () => {
  ...
  it('flushes Tower active-loads + wire by tenant prefix on LOAD_CREATED', async () => {
    await subscriber.handleDomainEvent(new DomainEvent(DOMAIN_EVENTS.LOAD_CREATED, '7', { loadNumber: 'LD-1' }));
  ...
  it('flushes only the active-loads prefix on LOAD_UPDATED (wire is unaffected)', async () => {
    await subscriber.handleDomainEvent(new DomainEvent(DOMAIN_EVENTS.LOAD_UPDATED, '7', { loadNumber: 'LD-1' }));
  ...
  it('flushes Tower prefixes on LOAD_ASSIGNED', async () => {
    await subscriber.handleDomainEvent(new DomainEvent(DOMAIN_EVENTS.LOAD_ASSIGNED, '3', {}));
  ...
  it('flushes Tower prefixes on LOAD_STATUS_CHANGED', async () => {
    await subscriber.handleDomainEvent(new DomainEvent(DOMAIN_EVENTS.LOAD_STATUS_CHANGED, '9', {}));
  ...
  it('flushes Tower prefixes on LOAD_STOP_STATUS_CHANGED', async () => {
    await subscriber.handleDomainEvent(new DomainEvent(DOMAIN_EVENTS.LOAD_STOP_STATUS_CHANGED, '9', {}));
  ...
  it('keeps the tenant prefix scoped so one tenant does not flush another', async () => {
    await subscriber.handleDomainEvent(new DomainEvent(DOMAIN_EVENTS.LOAD_CREATED, '1', {}));
